...
from .core.field_mapping import FieldMappingType
...
def read_csv(filepath: str, 
             ddf: DataFrame,
             field_mapping: Mapping[str, FieldMappingType],
    ...
    with open(filepath, encoding='utf-8') as sf:
        ...
        for field_name in fields_to_use:
            mapping_type = field_mapping[field_name]
            field_importer = mapping_type.importer(ddf.dataset.session, ddf, field_name, ts)
        ...
        for i, field_name in enumerate(csvf_fieldnames):
            mapping_type = field_mapping[field_name]
            column_offsets[i + 1] = column_offsets[i] + mapping_type.field_size * chunk_row_size
