...
class TestDataFrameDescribe(unittest.TestCase):
    ...
    def test_performance(self):
        ...
        with session.Session() as s:
            ...
            df.create_numeric('num', 'int32').data.write([i for i in range(1000000)])
            df.create_fixed_string('fs1', 6).data.write([b'abcdef' for i in range(1000000)])
            df.create_timestamp('ts1').data.write([1632234128 + i for i in range(1000000)])
            df.create_categorical('c1', 'int32', {'a': 1, 'b': 2}).data.write([1 for i in range(1000000)])
            df.create_indexed_string('is1').data.write(['abc' for i in range(1000000)])

            result = df.describe(include='all')
            expected = {'fields': ['num', 'fs1', 'ts1', 'c1', 'is1'], 'count': [1000000, 1000000, 1000000, 1000000, 1000000],
                        'mean': ['499999.50', 'NaN', '1632734127.50', 'NaN', 'NaN'], 'std': ['288675.13', 'NaN', '288675.13', 'NaN', 'NaN'],
                        'min': ['0.00', 'NaN', '1632234128.00', 'NaN', 'NaN'], '25%': ['2500.00', 'NaN', '1632236628.00', 'NaN', 'NaN'],
                        '50%': ['4999.99', 'NaN', '1632239127.99', 'NaN', 'NaN'], '75%': ['7499.99', 'NaN', '1632241627.99', 'NaN', 'NaN'],
                        'max': ['999999.00', 'NaN', '1633234127.00', 'NaN', 'NaN'], 'unique': ['NaN', 1, 'NaN', 1, 1],
                        'top': ['NaN', b'abcdef', 'NaN', 1, 'abc'], 'freq': ['NaN', 1000000, 'NaN', 1000000, 1000000]}
            self.assertEqual(result, expected)
