...
class TestdataFrameToPandas(unittest.TestCase):
    ...
    def test_to_pandas_df(self):
        ...
        with session.Session() as s:
            dst = s.open_dataset(bio, 'w', 'dst')
            df = dst.create_dataframe('df')
            df.create_numeric('num', 'int32').data.write(numeric_data)
            df.create_fixed_string('fs1', 1).data.write(fixed_str_data)
            df.create_timestamp('ts1').data.write(ts_data)
            df.create_categorical('c1', 'int32', {'a': 1, 'b': 2}).data.write(categorical_data)
            df.create_indexed_string('is1').data.write(idx_str_data)
            pddf1 = df.to_pandas()
            pddf2 = pd.DataFrame({'num':numeric_data, 'fs1':fixed_str_data, 'ts1':ts_data, 'c1':categorical_data, 'is1':idx_str_data}).astype(pddf1.dtypes)
    ...
    def test_to_pandas_df_row_filter(self):
        ...
        with session.Session() as s:
            ...
            pddf1 = df.to_pandas(row_filter=row_filter)
            pddf2 = pd.DataFrame({'num': numeric_data, 'fs1': fixed_str_data, 'ts1': ts_data, 'c1': categorical_data,
                                  'is1': idx_str_data}).astype(pddf1.dtypes).loc[row_filter].reset_index(drop=True)
            self.assertTrue(pddf1.equals(pddf2))
    ...
    def test_to_pandas_df_col_filter(self):
        ...
        with session.Session() as s:
            ...
            df.create_categorical('c1', 'int32', {'a': 1, 'b': 2}).data.write(categorical_data)
            df.create_indexed_string('is1').data.write(idx_str_data)
            pddf1 = df.to_pandas(col_filter='num')
            pddf2 = pd.DataFrame({'num': numeric_data}).astype(pddf1.dtypes)
            self.assertTrue(pddf1.equals(pddf2))

            pddf1 = df.to_pandas(col_filter=['num', 'fs1', 'ts1', 'c1'])
            pddf2 = pd.DataFrame({'num': numeric_data, 'fs1': fixed_str_data, 'ts1': ts_data, 'c1': categorical_data}).astype(pddf1.dtypes)
            self.assertTrue(pddf1.equals(pddf2))
    ...
    def test_to_pandas_df_filter(self):
        ...
        with session.Session() as s:
            ...
            pddf1 = df.to_pandas(row_filter=row_filter, col_filter=col_filter)
            pddf2 = pd.DataFrame({'num': numeric_data, 'fs1': fixed_str_data, 'ts1': ts_data,
                                  'c1': categorical_data}).astype(pddf1.dtypes).loc[row_filter].reset_index(drop=True)
            self.assertTrue(pddf1.equals(pddf2))
