...
def distinct(field, filter=None, timestamp=datetime.now(timezone.utc)):
    # raise NotImplementedError()
    np.unique(field)
#     d = Series(field)
#     distinct_values = set()
...
def get_trash_folder(group):
    ...
    while group.name != '/':
        group = group.parent
    if 'trash' not in group.keys():
        trash = group.create_group('trash')
    else:
        trash = group['trash']
