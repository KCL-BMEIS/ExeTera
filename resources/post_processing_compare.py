...
with h5py.File(old_source, 'r') as hf1:
	old_fields_set = set(hf1['vaccine_doses'].keys())

with h5py.File(new_source, 'r') as hf2:
	new_fields_set = set(hf2['vaccine_doses'].keys())
...
with h5py.File(old_source, 'r') as hf1:
	...
	for field in sorted_field_list:
		for k in list(hf1['vaccine_doses'][field].keys()):
			key = field + '@' + k
			value = hf1['vaccine_doses'][field][k][:]
			d1[key] = value

with h5py.File(new_source, 'r') as hf2:
	d2 = {}
	for field in sorted_field_list:
		for k in list(hf2['vaccine_doses'][field].keys()):
			key = field + '@' + k
			value = hf2['vaccine_doses'][field][k][:]
			d2[key] = value
